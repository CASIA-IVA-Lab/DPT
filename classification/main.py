...
def get_args_parser():
    ...
    #zychen
    parser.add_argument('--test_interval', type=int, default=0, help='Training with Evaluation')
    return parser
...
def main(args):
    ...
    print("Model : %s, max_epoch_dp_warm_up: %d"%(args.model, max_epoch_dp_warm_up))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.test_interval > 0 and epoch % args.test_interval == args.test_interval - 1 or epoch >= args.epochs - 10:
            test_stats = evaluate(data_loader_val, model, device)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
